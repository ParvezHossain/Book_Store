...
public class cart {
	...
	public void listItemsOfCart() throws SQLException, ClassNotFoundException {
		...
		for (int i = 0; i < id.size(); i++) {
			...
			String getItemName = "SELECT  `product-name` ,  `category-name` ,  `price` FROM  `products` WHERE  `product_id` ="
					+ getItemId + "";
			/*
			 * SELECT `product-name` , `category-name` , `price` FROM `products`
			 * WHERE `product_id` =1
			 */
	...
	public String getProductCategory(int id) throws SQLException, ClassNotFoundException {
		...
		Statement st = c.createStatement();
		/*
		 * SELECT `category-name` FROM `products` WHERE `product_id` =1
		 */
	...
	public double getTotalPriceOfCart() throws SQLException, ClassNotFoundException {
		ArrayList<Double> t_prices = new ArrayList(); // this will hold the
														// prices
		...
		for (int i = 0; i < id.size(); i++) {
			...
			rs.next();
			// theres only one value in resultset so go one step further and get
			// the value
	...
	public boolean removeProduct(int productId) {
		...
		/*
		 * 
		 * //Is there a product in cart with that id? if
		 * (this.id.contains(productId)) { //where is the id's position
		 * //this.id.indexOf(productId); int index = this.id.indexOf(productId);
		 * int quantity = this.qty.get(index); if (quantity == 1) { //remove the
		 * entire stack from all the ArrayLists
		 * 
		 * this.productCategory.remove(index); this.productName.remove(index);
		 * this.prices.remove(index); this.qty.remove(index);
		 * this.id.remove(index);
		 * 
		 * } else { //Update quantity and decrement by one quantity -= 1;
		 * this.qty.set(index, quantity); } removed = true; } else { removed =
		 * false; }
		 */
	...
	public static void main(String args[]) throws SQLException, ClassNotFoundException {
		try {
			...
			/*
			 * do { System.out.println("Do u wish to continue?"); String choice
			 * = sc.next(); }while (choice.equals("1"));
			 */
			String choice = "1";
			...
			for (int i = 0; i < productNames.size(); i++) {
				System.out.println(p_Category.get(i) + " " + productNames.get(i) + "              \t\t" + prices1.get(i)
						+ " \t\t" + qty1.get(i) + " \t\t" + prices1.get(i) * qty1.get(i));
			}
