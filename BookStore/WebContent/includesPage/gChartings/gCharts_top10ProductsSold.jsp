...
<body>
	...
	<%
		...
		*/
		...
		String top10products = "SELECT SUM( `product_quantity` ) AS solds,  `product_name` " + " FROM  `sales` "
				+ " GROUP BY  `product_name` " + " ORDER BY solds DESC  " + " LIMIT 0 , 10 ;";
	...
	<div class="grid_16" style="padding: 10px 0px 10px 0px;" id="whiteBox">
		...
		<script type="text/javascript">
                  ...
                  <%out.print("([ " + "['Product Name', 'Units Sold' ], ");
			int i = 0;
			while (i <= hits.size() - 1) {
				if (i < hits.size() - 1) {
					out.println("['" + product.get(i) + " ',  " + hits.get(i) + "  ],");
				} else {
					out.println("['" + product.get(i) + "',  " + hits.get(i) + "  ] ");
				}
				i++;
